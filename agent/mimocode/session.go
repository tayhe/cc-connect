...
// mimocodeSession manages multi-turn conversations with the OpenCode CLI.
// Each Send() launches a new `mimocode run --format json` process
// with --session for conversation continuity.
type mimocodeSession struct {
	cmd               string
	extraArgs         []string // extra args from cmd, prepended before mimocode args
	workDir           string
	model             string
	mode              string
	agentName         string
...
func newMimocodeSession(ctx context.Context, cmd string, extraArgs []string, workDir, model, mode, agentName, resumeID string, extraEnv []string) (*mimocodeSession, error) {
...
func (s *mimocodeSession) readLoop(cmd *exec.Cmd, stdout io.ReadCloser, stderrBuf *bytes.Buffer) {
	...
	if stderrMsg != "" {
		...
		if strings.Contains(stderrMsg, "Session not found") {
			s.chatID.Store("")
			slog.Warn("mimocodeSession: cleared stale session ID")
		}
	...
	if s.expectingContinue.Load() {
		slog.Info("mimocodeSession: readLoop ended after compaction_continue, skipping EventResult", "session_id", s.CurrentSessionID())
		s.expectingContinue.Store(false)
		return
	}
...
func (s *mimocodeSession) handleText(raw map[string]any) {
	...
	if synthetic && metadata != nil {
		if cc, ok := metadata["compaction_continue"].(bool); ok && cc {
			slog.Info("mimocodeSession: compaction_continue detected, marking expectingContinue", "session_id", s.CurrentSessionID())
...
func (s *mimocodeSession) handleToolUse(raw map[string]any) {
	...
	if status == "completed" {
		...
		// mimocode exits without generating any follow-up text. Surface the rejection
		...
		// and needs guidance to use mode="yolo" or update mimocode settings.
