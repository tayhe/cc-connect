...
func writePersistentModelCache(t *testing.T, cachePath string, models []core.ModelOption, updatedAt time.Time) string {
	...
	defaultSnapshot := mimocodeModelDiscoverySnapshot{workDir: "."}
...
func writePersistentModelCacheWithSnapshot(t *testing.T, cachePath string, snapshot mimocodeModelDiscoverySnapshot, models []core.ModelOption, updatedAt time.Time) string {
	t.Helper()
...
func waitForModelsInPersistentCache(t *testing.T, cachePath string, want []string) {
	...
	for time.Now().Before(deadline) {
		cache, err := loadMimocodePersistentModelCache(cachePath)
	...
	cache, err := loadMimocodePersistentModelCache(cachePath)
	if err != nil {
		t.Fatalf("loadMimocodePersistentModelCache(%q) error = %v", cachePath, err)
	}
...
func TestAgent_Name(t *testing.T) {
	a := &Agent{}
	if got := a.Name(); got != "mimocode" {
		t.Errorf("Name() = %q, want %q", got, "mimocode")
	}
}
...
func TestProjectModelCachePath_SanitizesProjectName(t *testing.T) {
	got := mimocodeProjectModelCachePath("/tmp/data", " ../team/demo project:alpha?beta ")
	if filepath.Dir(got) != filepath.Join("/tmp/data", "projects") {
		t.Fatalf("mimocodeProjectModelCachePath() dir = %q, want %q", filepath.Dir(got), filepath.Join("/tmp/data", "projects"))
	}
	base := filepath.Base(got)
	if !strings.HasPrefix(base, "team-demo-project-alpha-beta-") {
		t.Fatalf("mimocodeProjectModelCachePath() base = %q, want sanitized prefix", base)
	}
	if !strings.HasSuffix(base, ".mimocode-models.json") {
		t.Fatalf("mimocodeProjectModelCachePath() base = %q, want .mimocode-models.json suffix", base)
	}
	hashSuffix := strings.TrimSuffix(strings.TrimPrefix(base, "team-demo-project-alpha-beta-"), ".mimocode-models.json")
	if len(hashSuffix) != 16 {
		t.Fatalf("mimocodeProjectModelCachePath() hash suffix = %q, want 16 hex chars", hashSuffix)
	}
}

func TestProjectModelCachePath_DistinguishesSanitizeCollisions(t *testing.T) {
	pathA := mimocodeProjectModelCachePath("/tmp/data", "team/demo")
	pathB := mimocodeProjectModelCachePath("/tmp/data", "team-demo")
	if pathA == pathB {
		t.Fatalf("mimocodeProjectModelCachePath() collision: %q == %q", pathA, pathB)
	}
}

func TestLoadPersistentModelCache_NormalizesModels(t *testing.T) {
	cachePath := mimocodeProjectModelCachePath(t.TempDir(), "demo")
	...
	cache, err := loadMimocodePersistentModelCache(cachePath)
	if err != nil {
		t.Fatalf("loadMimocodePersistentModelCache() error = %v", err)
	}
	if cache == nil {
		t.Fatal("loadMimocodePersistentModelCache() = nil, want cache")
	}
...
func TestNew_SurfacesPersistentModelCacheViaAvailableModels(t *testing.T) {
	dataDir := t.TempDir()
	cachePath := mimocodeProjectModelCachePath(dataDir, "demo")
...
func TestAvailableModels_PersistsDiscoveryOnColdStart(t *testing.T) {
	...
	if cache == nil {
		t.Fatalf("loadMimocodePersistentModelCache(%q) = nil, want persisted cache", cachePath)
	}
...
func TestStartInitialModelRefresh_PrewarmsColdStartCacheAfterProviderWiring(t *testing.T) {
	...
	if cache, err := loadMimocodePersistentModelCache(cachePath); err != nil {
		t.Fatalf("loadMimocodePersistentModelCache(%q) error = %v", cachePath, err)
	} else if cache != nil {
...
func TestAvailableModels_IgnoresPersistentCacheForProviderMismatch(t *testing.T) {
	...
	providerASnapshot := func() mimocodeModelDiscoverySnapshot {
		a.activeIdx = 0
	...
	a.persistentModelCache = &mimocodePersistentModelCache{Models: []core.ModelOption{{Name: "cached/provider-a"}}, UpdatedAt: time.Now(), ProviderKey: providerASnapshot.providerKey, ContextKey: modelDiscoveryContextKey(providerASnapshot)}
	if !a.SetActiveProvider("provider-b") {
		t.Fatal("SetActiveProvider(provider-b) = false, want true")
	}
...
func TestAvailableModels_BackgroundRefreshPersistsProviderKey(t *testing.T) {
	...
	a.persistentModelCache = &mimocodePersistentModelCache{Models: []core.ModelOption{{Name: "cached/model"}}, UpdatedAt: time.Now(), ProviderKey: providerAKey, ContextKey: modelDiscoveryContextKey(providerASnapshot)}
...
func TestAvailableModels_IgnoresPersistentCacheForSameProviderNameDifferentConfig(t *testing.T) {
	...
	a.persistentModelCache = &mimocodePersistentModelCache{Models: []core.ModelOption{{Name: "cached/stale"}}, UpdatedAt: time.Now(), ProviderKey: staleKey, ContextKey: modelDiscoveryContextKey(staleSnapshot)}
...
func TestAvailableModels_IgnoresPersistentCacheForWorkDirMismatch(t *testing.T) {
	...
	a.persistentModelCache = &mimocodePersistentModelCache{
