...
// Agent drives the MiMoCode CLI in headless mode using `mimocode run --format json`.
//
// Modes:
//   - "default": standard mode
//   - "yolo":    auto mode (mimocode run is auto by default in non-interactive mode)
type Agent struct {
	workDir              string
	model                string
	mode                 string
	cmd                  string   // CLI binary name, default "mimocode"
...
type mimocodeModelDiscoverySnapshot struct {
	cmd         string
	workDir     string
	providerEnv []string
	providerKey string
...
func New(opts map[string]any) (core.Agent, error) {
	...
	cmd, extraArgs := core.ParseCmdOpts(opts, "mimo")
	...
	modelCachePath := mimocodeProjectModelCachePath(ccDataDir, ccProject)
	persistentModelCache, err := loadMimocodePersistentModelCache(modelCachePath)
	if err != nil {
		slog.Warn("mimocode: load persistent model cache failed", "path", modelCachePath, "err", err)
	}

	if _, err := exec.LookPath(cmd); err != nil {
		return nil, fmt.Errorf("mimocode: %q CLI not found in PATH, install from: https://github.com/opencode-ai/opencode", cmd)
	}
...
func mimocodeProjectModelCachePath(dataDir, project string) string {
	if dataDir == "" || project == "" {
		return ""
	}
	...
	fileName := fmt.Sprintf("%s-%s.mimocode-models.json", prefix, hex.EncodeToString(hash[:8]))
...
func loadMimocodePersistentModelCache(path string) (*mimocodePersistentModelCache, error) {
	if path == "" {
		return nil, nil
	}
	...
	var cache mimocodePersistentModelCache
...
func (a *Agent) configuredModelsForSnapshot(snapshot mimocodeModelDiscoverySnapshot) []core.ModelOption {
	a.mu.RLock()
	defer a.mu.RUnlock()
...
func (a *Agent) modelDiscoverySnapshot() mimocodeModelDiscoverySnapshot {
	a.mu.RLock()
	defer a.mu.RUnlock()
	return mimocodeModelDiscoverySnapshot{
		cmd:         a.cmd,
		workDir:     a.workDir,
...
func modelDiscoveryContextKey(snapshot mimocodeModelDiscoverySnapshot) string {
	h := sha256.New()
...
func (a *Agent) persistentModelsForSnapshot(snapshot mimocodeModelDiscoverySnapshot) []core.ModelOption {
	a.mu.RLock()
	defer a.mu.RUnlock()
...
func (a *Agent) startPersistentModelRefresh(snapshot mimocodeModelDiscoverySnapshot, allowColdStart bool) {
	a.mu.Lock()
	...
	go func() {
		...
		if err := a.storePersistentModelCache(snapshot, models); err != nil {
			slog.Warn("mimocode: update persistent model cache failed", "path", snapshot.cachePath, "err", err)
		}
	}()
}
...
func (a *Agent) storePersistentModelCache(snapshot mimocodeModelDiscoverySnapshot, models []core.ModelOption) error {
	models = normalizeModelOptions(models)
	if len(models) == 0 {
		return nil
	}

	cache := &mimocodePersistentModelCache{
		Models:      models,
		UpdatedAt:   time.Now(),
...
func (a *Agent) discoverModelsWithSnapshot(ctx context.Context, snapshot mimocodeModelDiscoverySnapshot) []core.ModelOption {
	...
	out, err := c.Output()
	if err != nil {
		slog.Debug("mimocode: discoverModels failed", "err", err)
		return nil
	}
	...
	models = normalizeModelOptions(models)
	if len(models) == 0 {
		slog.Debug("mimocode: discoverModels: no models in output")
		return nil
	}
	return models
}
...
func (a *Agent) AvailableModels(ctx context.Context) []core.ModelOption {
	...
	if models := a.discoverModelsWithSnapshot(ctx, snapshot); len(models) > 0 {
		if err := a.storePersistentModelCache(snapshot, models); err != nil {
			slog.Warn("mimocode: persist discovered model cache failed", "path", a.modelCachePath, "err", err)
		}
		return models
	}
...
func (a *Agent) StartSession(ctx context.Context, sessionID string) (core.AgentSession, error) {
	...
	return newMimocodeSession(ctx, cmd, extraArgs, workDir, model, mode, agentName, sessionID, extraEnv)
}

// ListSessions runs `mimocode session list` and parses the JSON output.
func (a *Agent) ListSessions(_ context.Context) ([]core.AgentSessionInfo, error) {
	a.mu.RLock()
	cmd := a.cmd
	workDir := a.workDir
	a.mu.RUnlock()
	return listMimocodeSessions(cmd, workDir)
}

func (a *Agent) Stop() error { return nil }

// DeleteSession implements core.SessionDeleter via `mimocode session delete <id>`.
...
// mimocodeSessionEntry represents a session from `mimocode session list` output.
...
func listMimocodeSessions(cmd, workDir string) ([]core.AgentSessionInfo, error) {
...
// MiMoCode's local database. Returns an empty map on any failure.
func querySessionMessageCounts() map[string]int {
	...
	if err != nil {
		slog.Warn("mimocode: sqlite3 CLI not found, message counts unavailable", "err", err)
		return nil
	}
	...
	if err != nil {
		slog.Warn("mimocode: sqlite3 query failed", "db_path", dbPath, "err", err)
		return nil
	}
...
func mimocodeDBPath() string {
	if xdg := os.Getenv("XDG_DATA_HOME"); xdg != "" {
		return filepath.Join(xdg, "mimocode", "mimocode.db")
	}
