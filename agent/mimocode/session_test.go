...
//
//	json: cannot unmarshal number into Go struct field mimocodeSessionEntry.updated of type string
...
func TestNewOpencodeSession_ContinueSessionTreatedAsFresh(t *testing.T) {
	s, err := newMimocodeSession(context.Background(), "echo", nil, "/tmp", "", "default", "", core.ContinueSession, nil)
	if err != nil {
		t.Fatalf("newMimocodeSession: %v", err)
	}
	defer s.Close()
...
func TestOpencodeSessionBuildRunArgsIncludesImagesAsFiles(t *testing.T) {
	s := &mimocodeSession{workDir: "/repo", model: "provider/model"}
...
func TestHandleStepStopSendsEventResult(t *testing.T) {
	...
	defer cancel()
	s := &mimocodeSession{events: make(chan core.Event, 1), ctx: ctx}
...
func TestHandleToolUsePermissionDeniedEmitsEventText(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	s := &mimocodeSession{events: make(chan core.Event, 4), ctx: ctx}
	s.handleToolUse(raw)
